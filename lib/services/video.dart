...
//   Users a= await FirebaseFirestore.instance.collection('users').where('uid' , isEqualTo: id).get().then(
//             (QuerySnapshot querySnapshot) { 
//                return Users(id:'1', phone: '11111111', name: querySnapshot.docs[0]['name'],
//                Follower:querySnapshot.docs[0]['Follower'] ,Following: querySnapshot.docs[0]['Following'],
//                email:querySnapshot.docs[0]['email'] ,like: querySnapshot.docs[0]['likes'],nickname: querySnapshot.docs[0]['nickname']);
//             }
...
Future<bool> updateComment(String id,String token,String content,int sl) async{
    try {
   ...
   }).then((value) => value.update({'cid':value.id}));
 ...
 Future<List<Video>> fetchProducts(int pagination) async {
...
return _item;
 ...
 Future<List<Video>> fetchProductsOfUser(String id) async {
 final List<Video> _item = [];
 
try {
  CollectionReference videos = FirebaseFirestore.instance.collection('videos');
          print(videos);
   await videos.where('uid',isEqualTo:id ).get().then((QuerySnapshot querySnapshot) {
...
,
  ...
  //   });
    // print('search------------------------------111111112222222222222222222222222222222222222222222222222222222222 ${search}');

    // print('search------------------------------111111112222222222222222222222222222222222222222222222222222222222 ${removeDiacritics(search).toLowerCase()}');
  ...
  //   },);
    ...
    for (var i = 0; i < _item.length ; i++) {
      print(_item[i].title);
       if( _item[i].title.contains('A')){
        print('7777777777777777777777777777777777777${_item[i]}');
       };
    }
      if(search!='')
       return _item.where((element) => element.title.contains(removeDiacritics(search))).toList();
       return _item;
// print(_item);
...
return _item;
 ...
 Future<List<Comment>> fetchComment(String videoId) async {  
 final List<Comment> _item = [];
...
// final filters =filterByUser ? 'orderBy="creatorId"&equalTo="$userId"' : '';
  CollectionReference comments = FirebaseFirestore.instance.collection('comments');
          print(comments);
   await comments.where('vid',isEqualTo: videoId).get().then((QuerySnapshot querySnapshot) {
   querySnapshot.docs.forEach((element) {
      print('---------------------------------------------------------');
      print(element['cid']);
      print('---------------------------------------------------------');

  // final user =element['u'] as Map<String,dynamic>;
  //   print('user ${user['id']}');
...
// final VideosMap = json.decode(response.body) as List<dynamic>;
// print(VideosMap);

//    VideosMap.forEach((element) {
...
//       _item.add(
//         Comment(
//           id:element['r']['id'].toString(),
//           userid:element['r']['uid'],
//           content:  element['r']['text'],
//           dateTime: element['r']['timestamp'],
//           likes: element['r']['likes'],
//           videoid: element['r']['vid'],
//           children: element['r']['childrens']
...
Future<void> postVideo(String title,String url,String id){
    CollectionReference videos = FirebaseFirestore.instance.collection('videos');
     return videos
          .add({
            'desc':title,
